...
export function useAppointmentsCrud() {
  ...
  useEffect(() => {
    ...
    if (userData?.token) {
      ...
      fetchSpecialties();
      fetchPatients(); // Load patients on initial render
    }
  }, []);

  // Fetch appointments for current doctor only
  ...
  // Fetch filtered appointments for current doctor
  const fetchFilteredAppointments = async (startDate?: string, endDate?: string, status?: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/appointments/filter?${params.toString()}`,
        {
          headers: { 
            'Authorization': `Bearer ${userData?.token}`,
            'Content-Type': 'application/json'
  ...
  // Fetch appointment types by specialty
